...
# INSTRUCTIONS
#
# Before running this app, you must make the following changes
#
# 1) Download these two files: "motus-projects.csv" and "receiver-deployments.csv"
#     - https://motus.org/data/downloads
# 2) Modify line 21 so it points to where these files are stores. Your Motus data should also be contained here.
# 3) Download your Motus database using the 'makeReport.r' file if it isn't already.
# 4) Select the default project ID by modifying line 24
# 5) Select a default station (by name) by modifying line 27.
# 6) Set your Google Authentication key (if it already hasn't been set) using line 31. This only has to be done once.
#
# Be patient as some plots take time to load!
#
...
names(projList)[which(is.na(names(projList)))] <- ""
  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      tabsetPanel(
          ...
          tabPanel('Antennas',
                   paste('Current antenna position'),
